...
from inserir_dados import insercao_banco
...
def get_users():
    ...
    response = mycursor.execute("SELECT nome, id_paciente FROM paciente")
...
altura = 0
pacientes = get_users()
paciente = st.selectbox("Nome do paciente:", [x[0] for x in pacientes],index=None)
id_paciente = 0
if paciente:
    id_paciente = [x[1] for x in pacientes if x[0] == paciente][0]
cuidador = st.selectbox("Nome do cuidador:", [x[0] for x in get_paciente_cuidador(paciente)])
medico = st.selectbox("Nome do médico:", [x[0] for x in get_paciente_medico(paciente)])
...
if imagem_upload is not None:
    ...
    for idx,x in enumerate(resultado):
        resultado[idx] = round(float(x), 1)
        st.write(str(resultado[idx]))
    st.write("O resultado da extração acima está correto?")
    left_column, right_column = st.columns(2)
    botao_sim = left_column.button("Sim")
    botao_nao = right_column.button("Não")
    if not (botao_nao or  botao_sim):
        st.stop()

    if(botao_nao):
        st.write("Tire outra foto do dispositivo e tente novamente.")
        st.rerun()
    ...
    if(botao_sim):
        ...
        st.write(parecer)
        resultado.append(parecer)
        insercao = insercao_banco(resultado,id_paciente,equipamento)
        st.write(insercao)
else:
